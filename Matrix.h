...
namespace gti320
{
	...
	class Matrix : public MatrixBase<Scalar, RowsAtCompile, ColsAtCompile>
	{
	public:
		/**
		 * Constructeur par défaut
		 */
		Matrix() : MatrixBase<Scalar, RowsAtCompile, ColsAtCompile>()
		{
		}
		 ...
		 */
		Matrix(const Matrix& other) : MatrixBase<Scalar, RowsAtCompile, ColsAtCompile>(other)
		{
		}
		 ...
		 */
		explicit Matrix(int rows, int cols) : MatrixBase<Scalar, RowsAtCompile, ColsAtCompile>(rows, cols)
		{
		}
		 ...
		 */
		Matrix(const std::initializer_list<std::initializer_list<Scalar>>& initializerList) : MatrixBase<Scalar, RowsAtCompile, ColsAtCompile>()
		{
		...
		Matrix& operator=(const SubMatrix<OtherScalar, OtherRows, OtherCols, RowStorage>& submatrix)
		{
			ASSERT(RowsAtCompile == Dynamic || submatrix.rows() == this->rows(), "Trying to assign a matrix with a submatrix with inconsistent rows");
			ASSERT(ColsAtCompile == Dynamic || submatrix.cols() == this->cols(), "Trying to assign a matrix with a submatrix with inconsistent cols");
		 ...
		 */
		Matrix<Scalar, RowsAtCompile, ColsAtCompile, RowStorage> transpose() const
		{
			...
			{
				return this->transpose<Scalar, RowsAtCompile, ColsAtCompile, RowStorage>();
			}
		...
		template<typename OtherScalar, int OtherRows, int OtherCols, int OtherStorage>
		friend std::ostream& operator<< (std::ostream& out, const Matrix<OtherScalar, OtherRows, OtherCols, OtherStorage>& matrix);
	...
	class Matrix<Scalar, RowsAtCompile, ColsAtCompile, RowStorage> : public MatrixBase<Scalar, RowsAtCompile, ColsAtCompile>
	{
	public:
		/**
		 * Constructeur par défaut
		...
		* Opérateur de copie a partir d'une sous-matrice.
		*/
		template <typename OtherScalar, int OtherRows, int OtherCols, int OtherStorage = RowStorage>
		Matrix& operator=(const SubMatrix<OtherScalar, OtherRows, OtherCols, OtherStorage>& submatrix)
		{
			ASSERT(RowsAtCompile == Dynamic || submatrix.rows() == this->rows(), "Trying to assign a matrix with a submatrix with inconsistent rows");
			ASSERT(ColsAtCompile == Dynamic || submatrix.cols() == this->cols(), "Trying to assign a matrix with a submatrix with inconsistent cols");

			// On parcours dans la direction des lignes puisque la sous matrice à copier devrait être une sous matrice par lignes
			this->resize(submatrix.rows(), submatrix.cols());
		 ...
		 */
		template <typename OtherScalar, int OtherRows, int OtherCols>
		Matrix& operator=(const SubMatrix<OtherScalar, OtherRows, OtherCols, ColumnStorage>& submatrix)
		{
			ASSERT(RowsAtCompile == Dynamic || submatrix.rows() == this->rows(), "Trying to assign a matrix with a submatrix with inconsistent rows");
			ASSERT(ColsAtCompile == Dynamic || submatrix.cols() == this->cols(), "Trying to assign a matrix with a submatrix with inconsistent cols");
			...
			// On parcours dans la direction des colonnes puisque la sous matrice à copier est une sous matrice par colonnes
			for (auto j = 0; j < submatrix.cols(); ++j)
			{
				for (auto i = 0; i < submatrix.rows(); ++i)
				{
		 ...
		 * Retourne la transposée de la matrice pour une matrice de type de stockage inverse
		 */
		Matrix<Scalar, RowsAtCompile, ColsAtCompile, ColumnStorage> transpose() const
		{
	 ...
	 */
	template<typename Scalar, int Rows, int Cols, int StorageType>
	std::ostream& operator<< (std::ostream& out, const Matrix<Scalar, Rows, Cols, StorageType>& matrix)
	{
